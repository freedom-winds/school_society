...
def complete_revision(client, token, club_id, revision_id, lock_version=1, suffix="", category_id=None):
    category = client.get("/api/v1/public/categories").get_json()["data"][0]
    payload = {
        "revision_id": revision_id, "lock_version": lock_version, "name": f"物理实验社{suffix}", "category_id": category_id or category["id"],
