...
export function RevisionDiffViewer({ pending, current }: { pending: Revision; current?: Revision | null }) { const fields: Array<[keyof Revision, string]> = [['category', '社团类别'], ['name', '社团名称'], ['short_intro', '短简介'], ['recruitment_slogan', '招新语'], ['full_intro', '完整介绍'], ['advisor', '指导老师'], ['activity_time', '活动时间'], ['activity_location', '活动地点']]; const comparing = !!current; const value = (revision: Revision | null | undefined, key: keyof Revision) => key === 'category' ? revision?.category?.name || '未分类' : String(revision?.[key] || '—'); return <div className={`revision-diff ${comparing ? '' : 'read-only'}`}>{fields.map(([key, label]) => <div className={comparing && value(pending, key) !== value(current, key) ? 'changed' : ''} key={key}><span>{label}</span><div>{comparing && <p>{value(current, key)}</p>}<b>{value(pending, key)}</b></div></div>)}</div> }
