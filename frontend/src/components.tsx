...
export function ConfirmDialog({ title, content, onConfirm, danger = false, children }: { title: string; content: string; onConfirm: () => Promise<void> | void; danger?: boolean; children: React.ReactNode }) { const [open, setOpen] = useState(false); return <><span onClick={() => setOpen(true)}>{children}</span><Modal title={title} open={open} onCancel={() => setOpen(false)} onOk={async () => { await onConfirm(); setOpen(false); message.success('操作已完成') }} okButtonProps={{ danger }}>{content}</Modal></> }

export function RevisionDiffViewer({ pending, current }: { pending: Revision; current?: Revision | null }) { const fields: Array<[keyof Revision, string]> = [['name', '社团名称'], ['short_intro', '短简介'], ['recruitment_slogan', '招新语'], ['full_intro', '完整介绍'], ['advisor', '指导老师'], ['activity_time', '活动时间'], ['activity_location', '活动地点']]; return <div className="revision-diff">{fields.map(([key, label]) => <div className={pending[key] !== current?.[key] ? 'changed' : ''} key={key}><span>{label}</span><div><p>{String(current?.[key] || '—')}</p><b>{String(pending[key] || '—')}</b></div></div>)}</div> }

export function UserSearchSelect({ onSelect, label = '搜索用户' }: { onSelect: (id: number) => void; label?: string }) { const [keyword, setKeyword] = useState(''); const [users, setUsers] = useState<Array<{ id: number; display_name: string; username: string; role: string }>>([]); const search = async () => { const { get } = await import('./api/client'); setUsers(await get(`/users/search?keyword=${encodeURIComponent(keyword)}`)) }; return <div className="user-search"><label>{label}</label><div><input value={keyword} onChange={e => setKeyword(e.target.value)} placeholder="输入姓名或用户名" /><button className="button secondary small" onClick={search}>搜索</button></div>{users.length > 0 && <ul>{users.map(u => <li key={u.id}><span><b>{u.display_name}</b><small>@{u.username} · {u.role}</small></span><button className="button text small" onClick={() => { onSelect(u.id); setUsers([]); setKeyword(u.display_name) }}>选择</button></li>)}</ul>}</div> }
